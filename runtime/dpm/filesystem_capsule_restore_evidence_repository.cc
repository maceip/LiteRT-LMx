...
absl::StatusOr<ScopedFd> OpenRegularOwnerOnlyFile(
  ...
  if (fd < 0) {
    return absl::ErrnoToStatus(
        errno,
        absl::StrCat("Unable to open capsule evidence file ", path.string()));
  }
  ...
  if (fstat(fd, &file_stat) != 0) {
    ...
    close(fd);
    return absl::ErrnoToStatus(
        saved_errno, absl::StrCat("Unable to inspect capsule evidence file ",
                                  path.string()));
  }
  if (!S_ISREG(file_stat.st_mode)) {
    close(fd);
    return absl::FailedPreconditionError(absl::StrCat(
        "capsule evidence path is not a regular file: ", path.string()));
  }
  if (file_stat.st_uid != geteuid() ||
      (file_stat.st_mode & (S_IRWXG | S_IRWXO)) != 0) {
    close(fd);
    return absl::PermissionDeniedError(
        "capsule evidence files must be current-user-owned and "
        "owner-only.");
  }
  return ScopedFd(fd);
}

absl::StatusOr<ScopedFileLock> AcquireLock(const std::filesystem::path& path,
                                           bool exclusive) {
  ABSL_ASSIGN_OR_RETURN(
      ScopedFd fd,
      OpenRegularOwnerOnlyFile(path, O_RDWR | O_CREAT, S_IRUSR | S_IWUSR));
  int result;
  do {
  ...
  if (result != 0) {
    return absl::ErrnoToStatus(errno,
                               "Unable to lock capsule evidence repository.");
  }
...
absl::Status ValidateSecureDirectory(const std::filesystem::path& directory,
  ...
  if (S_ISLNK(directory_stat.st_mode) || !S_ISDIR(directory_stat.st_mode)) {
  ...
  if ((directory_stat.st_mode & S_IRWXU) != S_IRWXU) {
    return absl::PermissionDeniedError(absl::StrCat(
        description, " must be owner-readable, writable, and searchable."));
  }
...
absl::Status ValidateNoSymlinkPathComponents(
  ...
  for (const std::filesystem::path& component : absolute_path.relative_path()) {
    ...
    if (lstat(cursor.c_str(), &component_stat) != 0) {
      return absl::ErrnoToStatus(
          errno, "Unable to inspect a capsule evidence-root path component.");
    }
    if (S_ISLNK(component_stat.st_mode)) {
      return absl::FailedPreconditionError(
          "capsule evidence root must not traverse symbolic links.");
    }
  }
  return absl::OkStatus();
}
...
absl::Status SyncDirectory(const std::filesystem::path& directory) {
  ...
  if (fd < 0) {
    return absl::ErrnoToStatus(errno,
                               "Unable to open capsule evidence directory.");
  }
  ScopedFd scoped(fd);
  return SyncFd(fd, "capsule evidence directory");
}
...
absl::Status EnsureOwnedDirectory(const std::filesystem::path& parent,
  ...
  ABSL_RETURN_IF_ERROR(
      ValidateSecureDirectory(parent, "capsule evidence parent"));
  bool created = false;
  int result;
  do {
...
absl::Status ValidateRepositoryTree(
  ...
  ABSL_RETURN_IF_ERROR(ValidateSecureDirectory(root, "capsule evidence root"));
  ABSL_RETURN_IF_ERROR(ValidateSecureDirectory(
      product_directory, "capsule evidence product directory"));
  ABSL_RETURN_IF_ERROR(ValidateSecureDirectory(
      capture_records_directory, "capsule capture-evidence records directory"));
  return ValidateSecureDirectory(restore_records_directory,
                                 "capsule restore-evidence records directory");
}
...
absl::Status WriteAll(int fd, absl::string_view bytes) {
  ...
  while (offset < bytes.size()) {
    ...
    if (count < 0) {
      return absl::ErrnoToStatus(errno,
                                 "Unable to write capsule capture evidence.");
    }
    if (count == 0) {
      return absl::DataLossError(
          "Zero-byte write while publishing capsule capture evidence.");
    }
...
absl::Status ReadExactAt(int fd, char* output, size_t size) {
  ...
  while (offset < size) {
    ...
    do {
      count =
          pread(fd, output + offset, size - offset, static_cast<off_t>(offset));
    } while (count < 0 && errno == EINTR);
    if (count < 0) {
      return absl::ErrnoToStatus(errno,
                                 "Unable to read capsule capture evidence.");
    }
    if (count == 0) {
      return absl::DataLossError("capsule capture-evidence file is truncated.");
    }
    offset += static_cast<size_t>(count);
  }
  return absl::OkStatus();
...
absl::StatusOr<std::string> AllocateBytes(uint64_t size) {
  if constexpr (sizeof(size_t) < sizeof(uint64_t)) {
    if (size > (std::numeric_limits<size_t>::max)()) {
      return absl::ResourceExhaustedError(
          "capsule capture evidence exceeds addressable memory.");
    }
  }
  const size_t allocation_size = static_cast<size_t>(size);
  if (allocation_size > std::string().max_size()) {
    return absl::ResourceExhaustedError(
        "capsule capture evidence exceeds the string allocation limit.");
  }
  ...
  try {
    ...
    return absl::ResourceExhaustedError(
        "Unable to allocate capsule capture-evidence storage.");
  } catch (const std::length_error&) {
    return absl::ResourceExhaustedError(
        "capsule capture evidence exceeds the allocation limit.");
  }
#else
  return std::string(allocation_size, '\0');
#endif
...
absl::StatusOr<std::string> ReadWholeFile(const std::filesystem::path& path) {
  ABSL_ASSIGN_OR_RETURN(ScopedFd fd, OpenRegularOwnerOnlyFile(path, O_RDONLY));
  struct stat before;
  if (fstat(fd.get(), &before) != 0) {
    return absl::ErrnoToStatus(errno,
                               "Unable to inspect capsule capture evidence.");
  }
  if (before.st_size < 0 ||
      static_cast<uint64_t>(before.st_size) >
          kMaximumAuthenticatedCapsuleCaptureEvidenceEnvelopeBytes) {
    return absl::ResourceExhaustedError(
        "capsule capture-evidence file exceeds its storage bound.");
  }
  if ((before.st_mode & S_IRUSR) == 0 ||
      (before.st_mode & (S_IWUSR | S_IWGRP | S_IWOTH)) != 0) {
    return absl::PermissionDeniedError(
        "capsule capture-evidence records must be owner-readable and "
        "read-only.");
  }
  ...
  if (fstat(fd.get(), &after) != 0) {
    return absl::ErrnoToStatus(
        errno, "Unable to re-inspect capsule capture evidence.");
  }
  if (after.st_dev != before.st_dev || after.st_ino != before.st_ino ||
    ...
    return absl::DataLossError(
        "capsule capture evidence changed during its immutable read.");
  }
  return bytes;
}
...
absl::StatusOr<std::pair<ScopedFd, std::filesystem::path>> CreateTempFile(
  ...
  if (fd < 0) {
    return absl::ErrnoToStatus(
        errno, "Unable to create capsule evidence temporary file.");
  }
  ScopedFd scoped(fd);
  if (fchmod(fd, S_IRUSR | S_IWUSR) != 0) {
    ...
    return absl::ErrnoToStatus(
        saved_errno, "Unable to protect capsule evidence temporary file.");
  }
  ...
  if (descriptor_flags < 0 ||
    ...
    return absl::ErrnoToStatus(
        saved_errno,
        "Unable to mark capsule evidence temporary file close-on-exec.");
  }
  return std::make_pair(std::move(scoped),
                        std::filesystem::path(writable.data()));
}

std::filesystem::path CaptureRecordPath(const std::filesystem::path& directory,
                                        const Hash256& checkpoint_id,
                                        const Hash256& evidence_id) {
  return directory / absl::StrCat(checkpoint_id.ToHex(), "-",
                                  evidence_id.ToHex(), ".capture");
}

std::filesystem::path RestoreRecordPath(const std::filesystem::path& directory,
                                        const Hash256& checkpoint_id,
                                        const Hash256& evidence_id) {
  return directory / absl::StrCat(checkpoint_id.ToHex(), "-",
                                  evidence_id.ToHex(), ".restore");
}

absl::Status ValidateStoredCaptureIdentity(
    const CapsuleCaptureEvidence& evidence, const Hash256& checkpoint_id,
    const Hash256& evidence_id) {
  if (evidence.checkpoint_id != checkpoint_id ||
      evidence.evidence_id != evidence_id) {
    return absl::DataLossError(
        "capsule capture evidence does not match its composite "
        "checkpoint/evidence storage key.");
  }
  return absl::OkStatus();
}

absl::Status ValidateStoredRestoreIdentity(
    const CapsuleRestoreEvidence& evidence, const Hash256& checkpoint_id,
    const Hash256& evidence_id) {
  if (evidence.plan.checkpoint_id != checkpoint_id ||
      evidence.evidence_id != evidence_id) {
    return absl::DataLossError(
        "capsule restore evidence does not match its composite "
        "checkpoint/evidence storage key.");
  }
  return absl::OkStatus();
}
...
absl::Status PublishCreateOnce(
  ...
  // the immutable reader preserves its owner-only, no-follow, bounded-read,
  // and before/after inode checks.
  ...
  if (existing.ok()) {
    ...
    return absl::AlreadyExistsError(
        absl::StrCat("Conflicting authenticated bytes already exist for this ",
  ...
  ABSL_ASSIGN_OR_RETURN(
      auto temporary,
      CreateTempFile(directory, absl::StrCat(checkpoint_id.ToHex(), "-",
                                             evidence_id.ToHex())));
  ...
  if (status.ok()) {
    ...
    if (result != 0) {
      status = absl::ErrnoToStatus(
          errno,
          absl::StrCat("Unable to make ", description, " evidence read-only."));
    }
  }
  if (status.ok()) {
...
FilesystemCapsuleRestoreEvidenceRepository::
    ...
    : root_(std::move(root)),
      ...
      capture_records_directory_(product_directory_ / "capture-records"),
      restore_records_directory_(product_directory_ / "restore-records"),
      lock_path_(product_directory_ / "repository.lock") {}

absl::StatusOr<std::unique_ptr<FilesystemCapsuleRestoreEvidenceRepository>>
FilesystemCapsuleRestoreEvidenceRepository::Create(std::filesystem::path root) {
  if (root.empty() || !root.is_absolute()) {
    return absl::InvalidArgumentError(
        "Capsule evidence repository requires an absolute existing root.");
  }
  for (const std::filesystem::path& component : root.relative_path()) {
    if (component == "." || component == "..") {
      return absl::InvalidArgumentError(
          "Capsule evidence root must not contain dot components.");
    }
  }
  ABSL_RETURN_IF_ERROR(ValidateNoSymlinkPathComponents(root));
  ABSL_RETURN_IF_ERROR(ValidateSecureDirectory(root, "capsule evidence root"));
  auto repository = std::unique_ptr<FilesystemCapsuleRestoreEvidenceRepository>(
...
absl::Status FilesystemCapsuleRestoreEvidenceRepository::Initialize() {
  ABSL_RETURN_IF_ERROR(EnsureOwnedDirectory(
      root_, product_directory_, "capsule evidence product directory"));
  ABSL_RETURN_IF_ERROR(
      EnsureOwnedDirectory(product_directory_, capture_records_directory_,
                           "capsule capture-evidence records directory"));
  ABSL_RETURN_IF_ERROR(
      EnsureOwnedDirectory(product_directory_, restore_records_directory_,
                           "capsule restore-evidence records directory"));
  ABSL_ASSIGN_OR_RETURN(ScopedFileLock lock, AcquireLock(lock_path_, true));
  ABSL_RETURN_IF_ERROR(ValidateRepositoryTree(root_, product_directory_,
                                              capture_records_directory_,
                                              restore_records_directory_));
  return SyncDirectory(product_directory_);
}

absl::Status FilesystemCapsuleRestoreEvidenceRepository::PutCaptureIfAbsent(
  ...
  ABSL_RETURN_IF_ERROR(ValidateRepositoryTree(root_, product_directory_,
                                              capture_records_directory_,
                                              restore_records_directory_));
  const std::filesystem::path path = CaptureRecordPath(
      capture_records_directory_, evidence.checkpoint_id, evidence.evidence_id);
  return PublishCreateOnce<CapsuleCaptureEvidence>(
      capture_records_directory_, path, bytes, evidence.checkpoint_id,
      evidence.evidence_id, authentication,
      &DecodeAuthenticatedCapsuleCaptureEvidence,
      &ValidateStoredCaptureIdentity, "capsule capture evidence");
}

absl::StatusOr<CapsuleCaptureEvidence>
FilesystemCapsuleRestoreEvidenceRepository::GetCapture(
  ...
  if (IsZeroHash(checkpoint_id) || IsZeroHash(expected_evidence_id)) {
    return absl::InvalidArgumentError(
        "capsule capture-evidence lookup requires exact nonzero "
  ...
  ABSL_RETURN_IF_ERROR(ValidateRepositoryTree(root_, product_directory_,
                                              ...
                                              restore_records_directory_));
  const std::filesystem::path path = CaptureRecordPath(
      capture_records_directory_, checkpoint_id, expected_evidence_id);
  ABSL_ASSIGN_OR_RETURN(const std::string bytes, ReadWholeFile(path));
  ABSL_ASSIGN_OR_RETURN(
      CapsuleCaptureEvidence evidence,
      DecodeAuthenticatedCapsuleCaptureEvidence(bytes, authentication));
  ABSL_RETURN_IF_ERROR(ValidateStoredCaptureIdentity(evidence, checkpoint_id,
                                                     expected_evidence_id));
  return evidence;
}

absl::Status FilesystemCapsuleRestoreEvidenceRepository::PutRestoreIfAbsent(
  ...
  const std::filesystem::path path =
      RestoreRecordPath(restore_records_directory_, evidence.plan.checkpoint_id,
                        evidence.evidence_id);
  return PublishCreateOnce<CapsuleRestoreEvidence>(
      restore_records_directory_, path, bytes, evidence.plan.checkpoint_id,
      evidence.evidence_id, authentication,
      &DecodeAuthenticatedCapsuleRestoreEvidence,
      &ValidateStoredRestoreIdentity, "capsule restore evidence");
}

absl::StatusOr<CapsuleRestoreEvidence>
FilesystemCapsuleRestoreEvidenceRepository::GetRestore(
    const Hash256& checkpoint_id, const Hash256& expected_evidence_id,
    const FreshWorkerAuthentication& authentication) const {
  if (IsZeroHash(checkpoint_id) || IsZeroHash(expected_evidence_id)) {
    return absl::InvalidArgumentError(
        "capsule restore-evidence lookup requires exact nonzero "
        "checkpoint and evidence IDs.");
  }
  ABSL_RETURN_IF_ERROR(ValidateFreshWorkerAuthentication(authentication));
  ABSL_RETURN_IF_ERROR(ValidateRepositoryTree(root_, product_directory_,
  ...
  ABSL_RETURN_IF_ERROR(ValidateRepositoryTree(root_, product_directory_,
                                              capture_records_directory_,
                                              restore_records_directory_));
  const std::filesystem::path path = RestoreRecordPath(
      restore_records_directory_, checkpoint_id, expected_evidence_id);
  ABSL_ASSIGN_OR_RETURN(const std::string bytes, ReadWholeFile(path));
  ABSL_ASSIGN_OR_RETURN(
      CapsuleRestoreEvidence evidence,
      DecodeAuthenticatedCapsuleRestoreEvidence(bytes, authentication));
  ABSL_RETURN_IF_ERROR(ValidateStoredRestoreIdentity(evidence, checkpoint_id,
                                                     expected_evidence_id));
  return evidence;
}
