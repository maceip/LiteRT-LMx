...
#include "support/preprocessor/by_pass_image_preprocessor.h"
...
TEST(ByPassImagePreprocessorTest, PreprocessWithTensorBuffer) {
  ByPassImagePreprocessor preprocessor;
  ...
  LITERT_ASSERT_OK_AND_ASSIGN(auto result_tensor_buffer,
                              result_image.GetPreprocessedImageTensor());
  ...
  EXPECT_EQ(tensor_type.Layout().Dimensions(), Dimensions({1, 32, 32, 3}));
  ...
  EXPECT_EQ(result_tensor_buffer->Get(),
            test_input_image.GetPreprocessedImageTensor().value()->Get());
}

TEST(ByPassImagePreprocessorTest, PreprocessWithRawBytesFailed) {
  ByPassImagePreprocessor preprocessor;

  // Create an InputImage from raw bytes.
  std::string dummy_image_data = "\x01\x02\x03\x04";
  ...
  EXPECT_THAT(std::string(result.status().message()),
              HasSubstr("Input image is not preprocessed."));
}
